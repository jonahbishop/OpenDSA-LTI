$(function () {
        ...
        if (!odsaLookupData) {
          Plotly.d3.json("/course_offerings/time_tracking_lookup/" + ODSA_DATA.course_offering_id,
            function (err, data) {
              ...
              let { weeksDates, daysHash } = getWeeksDates(new Date(starts_on + "T23:59:59-0000"), new Date("2020-08-10T23:59:59-0000"))
    ...
    odsaStore.getItem(['odsaLookupData', currentDate].join('-'))
      .then(function (lookups) {
        ...
        for (var i = 0; i < users.length; i++) {
          ...
          for (var j = 0; j < randomTime.length; j++) {
            var openBrace = "("
  ...
  function getrandom(num, mul) {
    ...
    for (i = 0; i <= num; i++) {
      ...
      value.push(parseFloat(rand.toFixed(2)));
    }
    return value;
  }
